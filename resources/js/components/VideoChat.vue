    ...
    export default {
        ...
        created() {
            ...
            this.channel
                ...
                .joining(user => this.users.push({id: user.id, name: user.name}))
                .leaving(user => {
                ...
                .listenForWhisper("client-sdp", async message => {
                    ...
                    let confirmed = confirm(`You have a call from: ${message.from}. Would you like to answer?`);
                    if (!confirmed) {
                        return this.channel.whisper("client-reject", {"room": message.room, "rejected": this.id});
                    }

                    this.room = message.room;
                    await this.caller.setRemoteDescription(message.sdp)
                    ...
                    let answer = await this.caller.createAnswer()
        ...
        methods: {
            ...
            async callUser(user) {
                ...
                this.localUserMedia = stream;
                let offer = await this.caller.createOffer()
                await this.caller.setLocalDescription(offer)
